...
note: tested using YUN and DS1307, AT24C32 - The code is optimized for 
      smallest footprint.
note: uses I2C library by Wayne Truchsess. Here for user convenince.
*/
...
// main method, does all REST work
   byte I2Ctunnel_t::command(YunClient client) { 
    byte error = 0;
// ============== parsing REST commad   
...
// ======== more HEXdata: write
 ...
 error =  I2c.write(device, asize[0], address, bindata, count);    // do I2C write
 }
  else {                                             
// ============= no HEXdata: read 
 error =  I2c.read(device, asize[0], address, bindata, count);    // do I2C read
#ifdef I2C_DEBUG
	lastCommand[1] = 'R';
...
//    client.print('\n');
 ...
 #ifdef I2C_DEBUG
    if (error){
	 strcat(lastCommand, " ERROR: ");
	 strcat(lastCommand, String(error).c_str());
    }
#endif
  return error;
 }
