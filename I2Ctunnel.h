...
class I2Ctunnel_t {
  public:
  byte command(YunClient client);
  /*
  byte write(const byte dev_id, const char size_add, 
       const unsigned int data_add, const byte data[], const byte ndata);
  byte read( const byte dev_id, const char size_add, 
       const unsigned int data_add, byte data[], const byte ndata);
  private:
  byte startI2C(const byte dev_id, const char size_add, 
